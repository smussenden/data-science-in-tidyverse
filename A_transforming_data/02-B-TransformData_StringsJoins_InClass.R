...
#' No results. Or are there? What if we handle for case sensitivity.
...
#' Since `mean()` breaks when there are NA values, we need to fix that with `rm.na = TRUE`



#' We can even create our own custom functions (won't get into that in this session, though).  
#'   
#' Now what if we wanted to apply our mean to every column in the flipped data frame? There's `summarise_all()`
...
#'   
#'   
#' Enter `summarise_if()`. Let's just look for columns that are numeric, `is.numeric`
...
#' To mutate certain columns based on criteria, we use `mutate_if()`. Say converting character columns to lower case?
